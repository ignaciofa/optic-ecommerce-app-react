...
export function CartProvider({ children }) {
  ...
  const itemsInCart =
    cart.length > 0 ? cart.reduce((prev, act) => prev + act.quantity, 0) : 0;

  const isInCart = (idProduct) =>
    cart.find((item) => (item.id === idProduct ? true : false));
      ...
      setCart(
        cart.map((item) => {
          return item.id === idProduct
            ? { id: idProduct, quantity: item.quantity + quantity }
            : item;
        })
      );
    } else {
  ...
  const totalPrice =
    cartItems.length > 0
      ? cartItems.reduce((prev, act) => prev + act.quantity * act.price, 0)
      : 0;
